...
public class CheckSystemCaseDimensions : Validator
{
    ...
    {
        if (systemUnit != null && videoCard != null && systemUnit.DoesMotherboardFit(motherboard) && systemUnit.DoesVideoCardFit(videoCard))
        {
            return CheckNext(
                cpu,
                bios,
        ...
        return new IncompatibilityProblem("System Case dimensions incompatibility");
    }
}