...
public class CheckMotherboardCpuCompatibility : Validator
{
    ...
    {
        if (motherboard != null && motherboard.IsCompatible(cpu))
        {
            return CheckNext(cpu, bios, motherboard, cooler, ram, videoCard, ssd, hdd, systemUnit, powerUnit, wifiAdapter, xmpProfile);
        }
        else
        {
            return new IncompatibilityProblem("CPU and the motherboard incompatibility");
        }
    }
}